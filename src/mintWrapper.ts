...
import { initUmi } from "./utils/metaplex";
...
async function createMintTransaction(){
    ...
    const transaction = await constructMultipleMintTransaction({candyMachineAddress,collectionAddress,candyGuard,minter,label,numberOfItems,blockHash,lookupTable});
